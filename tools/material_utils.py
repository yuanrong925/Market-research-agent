...
def check_material_sufficiency(
    ...
    策略（2026-07 更新）：
      1. 采用「相对排序优先」—— 不依赖绝对阈值，看 top-N 内是否有可用文本
      2. BM25 和向量相似度使用两套独立阈值：
         - BM25 归一化分 > 0.05 即认为有词法匹配
         - 向量相似度 > 0.3 即认为有语义匹配
         - 任一满足即视为「相关」
      3. 短文档（总块数 ≤ 10）大幅放宽：只要有文本且非空即通过

    返回: {"sufficient": bool, "reason": str, "score": float}
    """
    if not top_k_chunks:
    ...
    # ===== 两套独立阈值：BM25 分 + 向量相似度 =====
    # BM25 归一化分 > 0.05 即认为有词法匹配（极低阈值，避免过滤掉术语匹配）
    # 向量相似度 > 0.3 即认为有语义匹配
    bm25_threshold = 0.05
...
def grade_issues(
    ...
    对 FactChecker 原始输出进行双重标注：
      - error_type: format_error | evidence_missing | evidence_fabricated | conclusion_contradicted
...
def classify_severity(issues: List[Dict[str, str]], error_ratio: float) -> Tuple[str, str]:
    """
    根据 SOP 6.22 分级判定标准进行分类。
    返回: (severity_level, action)
    """
