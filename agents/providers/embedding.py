...
class DashScopeEmbedding(EmbeddingInterface):
    ...
    def __init__(
        ...
        from agents.config import DASHSCOPE_API_KEY, EMBEDDING_BASE_URL, EMBEDDING_MODEL

        self.api_key = api_key or DASHSCOPE_API_KEY
        self.model_name = model_name or EMBEDDING_MODEL
        self.api_base = api_base or EMBEDDING_BASE_URL
    ...
    def __call__(self, input: Any) -> List[List[float]]:
        """ChromaDB 批量 embedding 接口"""
        # 严格对齐 ChromaDB 契约：文本数量 = 向量数量，不做展平
    ...
    def embed_query(self, *args, **kwargs) -> List[List[float]]:
        ...
        return result  # 返回 List[List[float]]，严格对齐 ChromaDB 契约
    ...
    def _call_api(self, texts: List[str]) -> List[List[float]]:
        ...
        # 严格对齐 ChromaDB 契约：texts 已是 List[str]，不做额外展平
...
def get_embedding(
    ...
    **kwargs,
    ...
    参数:
      provider: "dashscope" | "openai" | "local_bge"，不指定则从环境变量 EMBEDDING_PROVIDER 读取
      model_name: 模型名称，不指定则使用各后端的默认模型
